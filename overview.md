...
Способ получения ключа из ВЕБ-хук запроса определяется значением ${platform}. Для gitlab и github он должен быть указан для соответствующего вызова ВЕБ-хука из этих систем. Для internal, он <i>TBD...</i>
...
ВЕБ компонент возвращает вызвавшей его системе ОК (HTTP 200) в случае, если ему удалось запустить указанную job-у (т.е. не ждет ее завершения). Ошибку он вернет в случае, когда не пройдена авторизация, или нет прав на запуск этой job.
