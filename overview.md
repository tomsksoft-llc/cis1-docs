...
* Любая задача может иметь параметры.  
* Любая задача может запустить еще один скрипт.  
* Каждая задача - это исполняемый файл с точки зрения ОС.  
* Есть возможность асинхронного запуска задач.
* Задачи разбиты на группы - _проекты_. Каждый _проект_ - это отдельный каталог, он может быть отдельным репозитарием.
...
* Возможность задач скриптов по расписанию.
* Возможность задач скриптов по внешнему сигналу - вебхуку.
* Возможность задач скриптов из самих скриптов (организация цепочек - сессий).
...
* Разделение прав доступа к _проектам_ через WebUI.
...
<td>Набор исполняемых файлов, которые обеспечивают все функции системы, скрипты находятся в подкаталоге <code>core</code>.</td>
...
<td>Это один сеанс работы <i>задачи</i>, а также и результаты его выполнения, каждая сборка выполняется в отдельном подкаталоге внутри <i>задачи</i> (имя подкаталога сборки шесть цифр с лидирующими нулями - её номер).</td>
...
<td>Это набор исполняемых файлов, каждый из которых выполняет какую-либо функцию, которую нужно использоваться в скриптах <i>задач</i>. Библиотечный скрипт принимает аргументы из командной строки и возвращает 0 в случае успеха, и не 0 в случае ошибки. Т.е. из скрипта <i>задачи</i> библиотечный скрипт запускается как команда операционной системы.</td>
...
`--new_session` - запустить новую сессию, позволяет добиться асинхронного исполнения в скриптах, при этом отрисовка в webui не поддерживается.
...
Запуск _задачи_ по вебхукам обеспечивает компонент WebUI. Здесь описаны интерфейсы, которых должны придерживаться и веб-компонент и ядро CIS:

Для приема вебхука от внешней системы используется URL:
```
...
При старте _задачи_, вызванной по вебхуку ей передаются параметры:
