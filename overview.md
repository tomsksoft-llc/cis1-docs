...
Он предназначен для автоматизации и связывания в единую цепочку рутинных операций, которые присутствуют в жизненном цикле ПО. Это операции: сборки (build), развертывания (deploy), автотестирования (autotesting), диагностика состояния систем (по запросу и периодическая), и т.д.
...
│   ├── internal – специальный _проекты_, который включен прямо в CIS, это всякие служебные ее задачи
│   ├── project1
...
### Чтение параметров переданных _задачи_
...
Те параметры, которые не будут установлены перед стартом дочерней _задачи_, получат значения по умолчанию.
...
В нем лежит исполняемый файл который используетс для запуска _задачи_. Имя этого файла берется из файла конфигурации _задачи_: `job.conf`.  
Файл `job.params` - описание входных параметров _задачи_.  
...
Создается каталог сборки, в котором они и будет выполнятся, в него копируются: исполняемый файл _задачи_, файл `job.params` с конкретными значениями парметров. Скрипт _задачи_ запускается в этом каталоге.
...
Базовая работоспособность ядра может быть проверена запуском _задачи_ `pyinternal/core_test`, которая автоматически создается при развертывании стандартным скриптом. В случае удачного исполнения этой _задачи_ можно считать, что ядро корректно выполняет свои базовые функции.
