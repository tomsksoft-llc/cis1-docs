...
<td>несколько скриптов, которые обеспечивают все функции системы, скрипты находятся в подкаталоге `core`.</td>
...
<td>Подкаталог внутри каталога `jobs`, внутри него находятся принадлежащие ему задачи.</td>
...
<td>задачи могут запускать одна другую, таким образом организовывая цепочки выполнения - это и есть сессии. Первая задача в цепочке начинает сессию, и она же ее закрывает. При открытии сессии ей присваивается уникальный идентификатор. Для каждой сессии создается отдельный файл лога в каталоге `sessions`, имя лог-файла совпадает с идентификатором сессии.</td>
...
* `build_number` - номер ее билда
* `session_id` - идентификатор текущей сессии
* `parent_job_name` - имя родительской задачи
* `parent_job_build_number` - номер билда родительской задачи
...
* `last_job_build_number` - номер билда предыдущей выполненной job
...
В каталог билда записывается файл `session_id.txt` в который пришется идентификатор текущей сессии.

Весь стандартный вывод и вывод ошибок записывается в файл `output.txt`, а код завершения записывается в файл `exitcode.txt` каталога билда.
...
Способ получения ключа из ВЕБ-хук запроса определяется значением ${platform}. Для gitlab и github он должен быть указан для соответствующего вызова ВЕБ-хука из этих систем. Для internal, он *TBD...*
...
* *action=open_session session_id=\<session_id\>*
* *action=close_session session_id=\<session_id\>*
* *action=error_use_startjob_without_job_name*
* *action=error_try_to_start_unexisted_job job_name="\<job name\>"*
* *action=error_cant_find_script_file job_name="\<script file name\>"*
* *action=error_cant_mkdir_for_job_build job_name=\"$job_name\" build_dir=\"$build_dir\"*
* *action=start_projects_maintenance_loop*
* *action=delete_old_build job_dir/build_dir="\<build dir\>"*
* *action=critical_error desc="attempt stop session without active session" pid=$$ ppid=$PPID*
* *action=critical_error desc=\"attempt start nested session\" session_id=$session_id*
* *TBD*...
...
* *action=open_session pid=$$ ppid=$PPID*
* *action=close_session pid=$$ ppid=$PPID*
* *action=set_call_param param_name=$param_name param_value="$param_value" pid=$$ ppid=$PPID*
* *action=set_value value_name=$value_name value="$value" pid=$$ ppid=$PPID*
* *action=try_to_start_job job_name="$job_name"*
* *action=error_use_startjob_without_job_name*
* *action=error_try_to_start_unexisted_job job_name="$job_name"*
* *action=error_cant_find_script_file job_name="$job_name"*
* *action=error_cant_mkdir_for_job_build job_name="$job_name" build_dir="$build_dir"*
* *action=set_call_param_to_default param_name=$param_name param_value=$param_value*
* *action=start_job job_name=$job_name build_dir=$build_dir pid=$$ ppid=$PPID*
* *action=end_job job_name=$job_name build_dir=$build_dir exit_code=$res pid=$$ ppid=$PPID*
* *TBD*...
...
Для того, чтобы нотифицировать UI компоненту о событиях происходящих в CIS асинхронно у него имеется http интерфейс. CIS для того, чтобы сообщить UI компоненте об этих событиях делает HTTP запрос на специальный URL:localhost:*TBD*...
