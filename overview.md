...
Для чтения параметров, которые переданы задаче, используем внутри ее скрипта команду системы <i>$cis_base_dir/core/$getparam \<param name\></i>, которая выдаст на стандартый выход значение параметра, например:
...
Создается каталог билда, в котором они и будет выполнятся, в него копируются: исполняемый файл job-ы, файл job.param с конкретными значениями парметров, TBD(еще чего-то?). Скрипт задачи запускается в этом каталоге.

В каталог билда записывается файл <i>session_id.txt</i> в который пришется идентификатор текущей сессии.

Весь стандартный вывод и вывод ошибок записывается в файл <i>output.txt</i>, а код завершения записывается в файл <i>exitcode.txt</i> каталога билда.
